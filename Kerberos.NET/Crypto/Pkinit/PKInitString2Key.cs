    ...
    public static class PKInitString2Key
    {
            ...
            ReadOnlySpan<byte> serverNonce = default)
        {
            ...
            try
            {
                ...
                Span<byte> fill = stackalloc byte[sha1.HashSizeInBytes];
                bool success = sha1.TryComputeHash(Concat(0, x, concat), fill, out int bytesWritten);
                Debug.Assert(success && bytesWritten == fill.Length);
                ...
                for (var i = 0; i < length; i++)
                {
                    ...
                    else
                    {
                        ...
                        success = sha1.TryComputeHash(Concat((byte)count, concatSize, concat), fill, out bytesWritten);
                        Debug.Assert(success && bytesWritten == fill.Length);
